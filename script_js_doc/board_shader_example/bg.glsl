// Note: we can't have the string "v iew" here as it confuses the internal GLSL hacking code
//
// Created by Reinder Nijhoff 2014
// @reindernijhoff
//
// https://www.shadertoy.com/v iew/Xtf3zn
//
// car model is made by Eiffie
// shader 'Shiny Toy': https://www.shadertoy.com/v iew/ldsGWB
...
float map( const in vec3 p ) {
    ...
    vec3 p_m = vec3( mod( pd.x + floor( pd.z * 4. )*0.25, 0.5 ) - 0.25, pd.y, mod( pd.z, 0.25 ) - 0.125 );
	d = udRoundBox( p_m, vec3( 0.245,0.1, 0.12 ), 0.005 ); 
